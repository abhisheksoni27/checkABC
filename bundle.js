...
var _drawing = __webpack_require__(7);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

(0, _drawing.draw)();

document.querySelector('.checkButton').onclick = _model2.default;
document.querySelector('.clearButton').onclick = _drawing.clearCanvas;
...
// Shape? 28x28
var pixels = new Array(finalWidth * finalHeight);

for (var i = 0; i < finalWidth * finalHeight; i++) {
    pixels[i] = 0;
}
...
function infer() {
    loadModel().then(function (res) {
        model = res;
        tf.tidy(function () {
            var imageTensor = tf.tensor(pixels).reshape([-1, 28, 28, 1]);
            var output = model.predict(imageTensor);

            var axis = 1;
            var predictions = Array.from(output.argMax(axis).dataSync());
            var answer = String.fromCharCode(64 + predictions[0]);
            result.innerHTML = 'You drew ' + answer + '.';
        });
    });
}
...
function check() {
    ...
    // Get raw pixel data
    var dataSrc = resizeCtx.getImageData(0, 0, finalWidth, finalHeight).data;
...
new _ripple.MDCRipple(document.querySelector('.button'));
var WIDTH_MULTIPLIER = 0.8;
var HEIGHT_MULTIPLIER = 0.6;
...
var OnStartListener = function OnStartListener(e) {
    if (isMobile) {
        canvas.addEventListener('touchmove', onPaint, false);
    } else {
        canvas.addEventListener('mousemove', onPaint, false);
...
var OnMoveListener = function OnMoveListener(e) {
    e.preventDefault();
    last_mouse.x = mouse.x;
    last_mouse.y = mouse.y;
    mouse = getMousePos(canvas, e);
...
var OnEndListener = function OnEndListener() {
    if (isMobile) {
        canvas.removeEventListener('touchmove', onPaint, false);
        // Reset!
        mouse = {
